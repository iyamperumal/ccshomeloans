namespace CcsWeb.Controllers
{
    ...
    internal static class Util
    {
        ...
        internal static List<RefiOption> GetConvHarpOptionsOld_M2D10(CcsLocalDbContext db, Application app)
        {
            if (!RateSheetLoaded)
            {
            ...
            for (int i = 0; i < refiVariables.Count; i++)
            {
                ...
                {
                    nullable6 = refiVariables[i].MaxNumberOfUnits;
                    if ((nullable6.GetValueOrDefault() < 4) && nullable6.HasValue)
                    {
                        continue;
                    }
                }
            ...
            Label_03AE:
                ...
                RefiOption item = new RefiOption();
                decimal num3 = app.FirstMortgageBalance.Value;
                ...
                if (flag && flag2)
                {
                    ...
                    num3 += app.SecondMortgageBalance.Value;
                }
                ...
                double num7 = (refiVariables[i].MaxLTV / 100.0) * ((double) app.EstimatedHomeValue.Value);
                double num8 = (refiVariables[i].CLTV / 100.0) * ((double) app.EstimatedHomeValue.Value);
                maxLoanLimit = Math.Min(maxLoanLimit, num7);
                if (flag && !flag2)
                {
                    num8 -= (double) app.SecondMortgageBalance.Value;
                }
                maxLoanLimit = Math.Min(maxLoanLimit, num8);
                item.VarNum = refiVariables[i].OptionNumber;
                item.NewMI_MonthlyAmount = 0.00M;
                double num9 = refiVariables[i].MiFactor / 6.0;
                double upfrontMI = refiVariables[i].UpfrontMI;
                double num11 = (0.05 * refiVariables[i].NewInterestRate) / 360.0;
                item.OptionName = refiVariables[i].ScheduleName;
                item.DatePrepared = new DateTime?(DateTime.Today);
                ...
                decimal num12 = 0.00M;
                item.TotalFixedFees = new decimal?((((((((refiVariables[i].ProcessingFee + refiVariables[i].UnderwritingFee) + refiVariables[i].AppraisalFee) + refiVariables[i].CreditReportFee) + refiVariables[i].ClosingEscrowFee) + refiVariables[i].EndorsementsReconveyanceFee) + refiVariables[i].MortgageRecordingfee) + refiVariables[i].TaxServiceFee) + refiVariables[i].FloodCertificationFee);
                double num13 = (((((((refiVariables[i].OriginationPercent - refiVariables[i].LenderCreditPercent) + refiVariables[i].TitleInusrancePercent) + refiVariables[i].IntangibleTaxPercent) + refiVariables[i].StateTaxPercent) + refiVariables[i].DiscountPercent) + num11) + num9) + upfrontMI;
                numofMonthstoEscrowTaxes = num3;
                ...
                double num14 = (double) nullable12.Value;
                double num15 = num14 / (1.0 - num13);
                double num16 = (num15 / ((double) app.EstimatedHomeValue.Value)) * 100.0;
                if (num16 < 80.0)
                {
                    num13 = ((((((refiVariables[i].OriginationPercent - refiVariables[i].LenderCreditPercent) + refiVariables[i].TitleInusrancePercent) + refiVariables[i].IntangibleTaxPercent) + refiVariables[i].StateTaxPercent) + refiVariables[i].DiscountPercent) + num11) + upfrontMI;
                    num15 = num14 / (1.0 - num13);
                }
                if (num15 <= maxLoanLimit)
                {
                    double num17 = (double) app.TotalOfMonthlyPaymentsOnDebtToPayOff.Value;
                    double num18 = (maxLoanLimit * (1.0 - num13)) - num14;
                    if (app.LoanTypeRequested == LoanTypeRequestedEnum.DebtConsolidationPayOffCreditors)
                    {
                        item.TotalBalanceOfDebtToConsolidate = app.EstimateTotalDebtToPayOff;
                        item.ShowDebtConsal = true;
                        if (num18 < ((double) app.EstimateTotalDebtToPayOff.Value))
                        {
                            item.TotalBalanceOfDebtToConsolidate = new decimal?((decimal) num18);
                            num17 *= num18 / ((double) app.EstimateTotalDebtToPayOff.Value);
                            num3 += (decimal) num18;
                        }
                        else
                        {
                            num3 += item.TotalBalanceOfDebtToConsolidate.Value;
                        }
                        numofMonthstoEscrowTaxes = num3;
                        firstMortgagePayment = numofMonthstoEscrowTaxes + item.TotalFixedFees;
                        monthlyMortgageInsur = item.HazardInsuranceReserves;
                        nullable12 = ((firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable12 = null))) + item.CountyPropertyTaxReserves;
                        num14 = (double) nullable12.Value;
                        num13 = (((((((refiVariables[i].OriginationPercent - refiVariables[i].LenderCreditPercent) + refiVariables[i].TitleInusrancePercent) + refiVariables[i].IntangibleTaxPercent) + refiVariables[i].StateTaxPercent) + refiVariables[i].DiscountPercent) + num11) + num9) + upfrontMI;
                        num15 = num14 / (1.0 - num13);
                        if (num16 < 80.0)
                        {
                            num13 = ((((((refiVariables[i].OriginationPercent - refiVariables[i].LenderCreditPercent) + refiVariables[i].TitleInusrancePercent) + refiVariables[i].IntangibleTaxPercent) + refiVariables[i].StateTaxPercent) + refiVariables[i].DiscountPercent) + num11) + upfrontMI;
                    ...
                    double num19 = (maxLoanLimit * (1.0 - num13)) - num14;
                    double num20 = Math.Min(num19, refiVariables[i].MaxCashOut);
                    if (app.LoanTypeRequested == LoanTypeRequestedEnum.CashOutMortgage)
                    {
                        num12 = app.CashOutRequested.Value;
                        item.Cashout = app.CashOutRequested.Value;
                        item.ShowCashout = true;
                    }
                    else if (app.LoanTypeRequested == LoanTypeRequestedEnum.DebtConsolidationPayOffCreditors)
                    {
                        num12 = app.AdditionalCashOutRequested.Value;
                    ...
                    monthlyMortgageInsur = item.HazardInsuranceReserves;
                    nullable12 = ((firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable12 = null))) + item.CountyPropertyTaxReserves;
                    double num21 = (double) nullable12.Value;
                    if ((refiVariables[i].MortgageProgramOption.Trim() == "HARP_FannieBefore2009") || (refiVariables[i].MortgageProgramOption.Trim() == "HARP_FredieBefore2009"))
                    {
                        num21 += ((double) app.MonthlyMortgageInsur.Value) * 2.0;
                    }
                    double num22 = num21 / (1.0 - num13);
                    app.ProposedLoanAmount = (decimal) num22;
                    ...
                    if (v.FinalAdjustment >= -1000f)
                    {
                        ...
                        num11 = (0.05 * pricing.interest) / 360.0;
                        item.InterestRateSavings = item.CurrentInterestRate - item.InterestRate;
                        double? interestRateSavings = item.InterestRateSavings;
                        if ((interestRateSavings.GetValueOrDefault() > 0.0) && interestRateSavings.HasValue)
                        {
                        ...
                        if (flag && flag2)
                        {
                            num40 = ((((double) item.LoanBalance.Value) * item.CurrentInterestRate.Value) + (((double) item.SecondMtgBalance.Value) * app.SecondMortgageInterestRate.Value)) / (((double) item.LoanBalance.Value) + ((double) item.SecondMtgBalance.Value));
                            interestRateSavings = item.InterestRate;
                            item.BlendedRateSavings = interestRateSavings.HasValue ? new double?(num40 - interestRateSavings.GetValueOrDefault()) : ((double?) (nullable15 = null));
                            interestRateSavings = item.BlendedRateSavings;
                            if ((interestRateSavings.GetValueOrDefault() > 0.0) && interestRateSavings.HasValue)
                            {
                                item.ShowBlendedRate = true;
                        ...
                        else
                        {
                            ...
                            num40 = (double) item.LoanAmount.Value;
                            nullable15 = interestRateSavings.HasValue ? new double?(interestRateSavings.GetValueOrDefault() * num40) : null;
                            item.Credit = new decimal?((decimal) nullable15.Value);
                        }
                        num13 = (((((((refiVariables[i].OriginationPercent - item.CreditPoints.Value) + refiVariables[i].TitleInusrancePercent) + refiVariables[i].IntangibleTaxPercent) + refiVariables[i].StateTaxPercent) + item.DiscountPoints.Value) + num11) + num9) + upfrontMI;
                        if (num16 < 80.0)
                        {
                            num13 = ((((((refiVariables[i].OriginationPercent - item.CreditPoints.Value) + refiVariables[i].TitleInusrancePercent) + refiVariables[i].IntangibleTaxPercent) + refiVariables[i].StateTaxPercent) + item.DiscountPoints.Value) + num11) + upfrontMI;
                        }
                        numofMonthstoEscrowTaxes = num3;
                        firstMortgagePayment = numofMonthstoEscrowTaxes + item.TotalFixedFees;
                        monthlyMortgageInsur = item.HazardInsuranceReserves;
                        nullable12 = ((firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable12 = null))) + item.CountyPropertyTaxReserves;
                        num21 = (double) nullable12.Value;
                        if ((refiVariables[i].MortgageProgramOption.Trim() == "HARP_FannieBefore2009") || (refiVariables[i].MortgageProgramOption.Trim() == "HARP_FredieBefore2009"))
                        {
                            num21 += ((double) app.MonthlyMortgageInsur.Value) * 2.0;
                        }
                        num22 = num21 / (1.0 - num13);
                        item.LoanAmount = new decimal?((decimal) num22);
                        if (maxLoanLimit >= ((double) item.LoanAmount.Value))
                        {
                            item.NewMI_MonthlyAmount = 0;
                            item.PMI_MIP_VA_FFReserves = 0;
                            item.MI_Upfront_Fee = 0.00M;
                            double num24 = (num22 / ((double) app.EstimatedHomeValue.Value)) * 100.0;
                            item.LTV = num24;
                            item.CLTV = num24;
                            if ((app.Has2ndMortgage > HaveSecondMortgageEnum.No) && (((YesNoAns) app.PayOff2ndMortgage) == YesNoAns.No))
                            {
                                item.CLTV = ((num22 + ((double) app.SecondMortgageBalance.Value)) / ((double) app.EstimatedHomeValue.Value)) * 100.0;
                            }
                            int num25 = (int) (item.LTV * 1000000.0);
                            item.LTV = ((double) num25) / 1000000.0;
                            num25 = (int) (item.CLTV * 1000000.0);
                            item.CLTV = ((double) num25) / 1000000.0;
                            if (item.CLTV <= refiVariables[i].CLTV)
                            {
                                ...
                                else if (refiVariables[i].LoanType == LoanTypeEnum.ConventonalConforming)
                                {
                                    if (num24 >= 80.0)
                                    {
                                        item.MI_Upfront_Fee = new decimal?((decimal) (num22 * upfrontMI));
                                        item.PMI_MIP_VA_FFReserves = new decimal?((decimal) (num22 * num9));
                                        item.NewMI_MonthlyAmount = item.PMI_MIP_VA_FFReserves / 2.00M;
                                    }
                                }
                                ...
                                double num27 = ((((double) item.TotalNewPrincipalInterestPaymentWithMI.Value) + (((double) app.AnnualPropertyTaxes.Value) / 12.0)) + (((double) app.AnnualHomeownersInsur.Value) / 12.0)) + (((double) app.AnnualHomeownersAssocDues.Value) / 12.0);
                                ...
                                if (GetMaxPayment(refiVariables[i].MaxfrontDTI, refiVariables[i].MaxBacktDTI, monthlyIncome, monthlyDebt) >= num27)
                                {
                                    item.FrontDTI = (num27 / monthlyIncome) * 100.0;
                                    item.BackDTI = ((num27 + monthlyDebt) / monthlyIncome) * 100.0;
                                    decimal? nullable = 0M;
                                    ...
                                    numofMonthstoEscrowTaxes = (decimal) num11;
                                    item.DailyInterestCharges = numofMonthstoEscrowTaxes * item.LoanAmount;
                                    ...
                                    double num1 = (double) item.LoanAmount.Value;
                                    double local1 = item.CreditPoints.Value;
                                    firstMortgagePayment = item.LoanAmount;
                                    numofMonthstoEscrowTaxes = (decimal) refiVariables[i].OriginationPercent;
                                    if (firstMortgagePayment.HasValue)
                                    {
                                    ...
                                    numofMonthstoEscrowTaxes = item.FloodCertificationFee;
                                    decimal? nullable2 = firstMortgagePayment.HasValue ? new decimal?(firstMortgagePayment.GetValueOrDefault() + numofMonthstoEscrowTaxes) : null;
                                    ...
                                    firstMortgagePayment = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable12 = null));
                                    monthlyMortgageInsur = item.MI_Upfront_Fee;
                                    decimal? nullable3 = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable12 = null));
                                    firstMortgagePayment = nullable3;
                                    monthlyMortgageInsur = item.DailyInterestCharges;
                                    ...
                                    firstMortgagePayment = nullable2;
                                    monthlyMortgageInsur = item.HazardInsuranceReserves;
                                    firstMortgagePayment = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable12 = null));
                                    monthlyMortgageInsur = item.CountyPropertyTaxReserves;
                                    if (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue)
                                    ...
                                    firstMortgagePayment = item.LoanAmount;
                                    monthlyMortgageInsur = nullable3;
                                    if (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue)
                                    {
                                        new decimal?(firstMortgagePayment.GetValueOrDefault() * monthlyMortgageInsur.GetValueOrDefault());
                                    }
                                    double num41 = (double) item.LoanAmount.Value;
                                    double num42 = (double) num3;
                                    firstMortgagePayment = item.ProcessingFee;
                                    monthlyMortgageInsur = item.UnderwritingFee;
                                    firstMortgagePayment = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable12 = null));
                                    monthlyMortgageInsur = item.AppraisalFee;
                                    ...
                                    double num32 = (double) item.LoanAmount.Value;
                                    interestRateSavings = item.InterestRate;
                                    double num33 = (interestRateSavings.Value / 100.0) / 12.0;
                                    double num34 = (double) app.FirstMortgagePayment.Value;
                                    if (((YesNoAns) app.PayOff2ndMortgage) == YesNoAns.Yes)
                                    {
                                        num34 += (double) app.SecondMortgagePayment.Value;
                                    }
                                    double a = -Math.Log(1.0 - ((num32 / num34) * num33)) / Math.Log(1.0 + num33);
                                    item.NumberofPaymentstoMaturity = new int?((int) Math.Ceiling(a));
                                    ...
                                    maxNumberOfUnits = (maxNumberOfUnits.HasValue & numberofPaymentstoMaturity.HasValue) ? new int?(maxNumberOfUnits.GetValueOrDefault() - numberofPaymentstoMaturity.GetValueOrDefault()) : ((int?) (nullable6 = null));
                                    ...
                                    {
                                        int num38 = getMILTV(loanAmount, (double) item.TermInMonths.Value, item.InterestRate.Value, (double) item.NewMonthlyPaymentPrincipalInterest.Value, 78.0, (double) app.EstimatedHomeValue.Value);
                                        payment = (((((double) item.NewMonthlyPaymentPrincipalInterest.Value) + ((double) item.NewMI_MonthlyAmount.Value)) * num38) + (((double) item.NewMonthlyPaymentPrincipalInterest.Value) * (((double) item.TermInMonths.Value) - num38))) / 360.0;
                                    }
                                    item.APR = new double?(GetAPR(loanAmount, item.InterestRate.Value / 1200.0, (double) item.TermInMonths.Value, payment));
                                    item.PercentCharge /= 100.0;
                                    item.ShowBlendedRate = true;
        ...
        internal static double GetLoanAmountNeededAtThisPoint(Variable vrs, double amountNeeded, double totalFixed, double fiveDaysinterest, double MIFactorReserve, double MiUpfrontfactor, double estimatedHomeValue, double creditPoints, double discoutPoints)
        {
            double num = 0.0;
            double num2 = (((((((vrs.OriginationPercent - creditPoints) + discoutPoints) + vrs.TitleInusrancePercent) + vrs.IntangibleTaxPercent) + vrs.StateTaxPercent) + fiveDaysinterest) + MIFactorReserve) + MiUpfrontfactor;
            double num3 = amountNeeded + totalFixed;
            num = num3 / (1.0 - num2);
            double num4 = (num / estimatedHomeValue) * 100.0;
            if (num4 < 80.0)
            {
                num2 = ((((((vrs.OriginationPercent - creditPoints) + discoutPoints) + vrs.TitleInusrancePercent) + vrs.IntangibleTaxPercent) + vrs.StateTaxPercent) + fiveDaysinterest) + MiUpfrontfactor;
                num = num3 / (1.0 - num2);
            }
            return num;
        }

        internal static Variables GetLoanVariablesForRateSheet(Application app, Variable vrs, HiBalance hibalance)
        {
            ...
            if (flag && flag2)
            {
                if (vrs.LoanType == LoanTypeEnum.VA)
                {
                    TimeSpan span = (TimeSpan) (DateTime.Now - app.SecondMortgageOriginationDate.Value);
                    if (span.Days < 330)
                    {
            ...
            if (vrs.LoanType == LoanTypeEnum.USRDA)
            {
                variables.isUSDA = true;
            }
            if (vrs.LoanType == LoanTypeEnum.FHA)
            {
                variables.isFHA = true;
            }
            if ((vrs.LoanType == LoanTypeEnum.VA_IRRL) && ((app.LoanTypeRequested == LoanTypeRequestedEnum.RateAndTermRefiLowerPayment) || (app.LoanTypeRequested == LoanTypeRequestedEnum.RateAndTermRefiShorterTerm)))
            {
                variables.isVA = true;
            }
            if ((vrs.LoanType == LoanTypeEnum.FHA_Streamline) && ((app.LoanTypeRequested == LoanTypeRequestedEnum.RateAndTermRefiLowerPayment) || (app.LoanTypeRequested == LoanTypeRequestedEnum.RateAndTermRefiShorterTerm)))
            {
                variables.isFHA = true;
                variables.isStreamline = true;
            }
            if (vrs.LoanType == LoanTypeEnum.Jumbo)
            {
                variables.isJumbo = true;
            }
            variables.isHighBalance = hibalance.IsHibalance;
            if (((vrs.RateType == InterestRateTypeEnum.Adjustable_10_1) || (vrs.RateType == InterestRateTypeEnum.Adjustable_3_1)) || (((vrs.RateType == InterestRateTypeEnum.Adjustable_5_1) || (vrs.RateType == InterestRateTypeEnum.Adjustable_7_1)) || (vrs.RateType == InterestRateTypeEnum.InterstOnlyAdjustable)))
            {
            ...
            if (((vrs.RateType == InterestRateTypeEnum.Fixed) || (vrs.RateType == InterestRateTypeEnum.InterstOnlyFixed)) || (vrs.RateType == InterestRateTypeEnum.Balloon))
            {
                variables.isFixed = true;
            }
            if ((vrs.LoanType == LoanTypeEnum.FHA_Streamline) || (vrs.LoanType == LoanTypeEnum.VA_IRRL))
            {
            ...
            if (vrs.LoanType == LoanTypeEnum.HARP)
            {
            ...
            if (vrs.RateType == InterestRateTypeEnum.Adjustable_10_1)
            {
                variables.isYear10 = true;
            }
            else if (vrs.RateType == InterestRateTypeEnum.Adjustable_5_1)
            {
                variables.isYear5 = true;
            }
            else if (vrs.RateType == InterestRateTypeEnum.Adjustable_7_1)
            {
                variables.isYear7 = true;
            }
            else if (vrs.RateType == InterestRateTypeEnum.Fixed)
            {
                if (vrs.newTermInYears == 30)
                {
                    variables.isYear30 = true;
                }
                else if (vrs.newTermInYears == 0x19)
                {
                    variables.isYear25 = true;
                }
                else if (vrs.newTermInYears == 20)
                {
                    variables.isYear20 = true;
                }
                else if (vrs.newTermInYears == 15)
                {
                    variables.isYear15 = true;
                }
                else if (vrs.newTermInYears == 10)
                {
                    variables.isYear10 = true;
                }
            }
            if (vrs.LenderPaidComp > 0.0)
            {
                variables.lenderPaidMorIns = true;
            }
            variables.loanamount = (float) app.ProposedLoanAmount;
            variables.loanTerm = vrs.newTermInYears;
        ...
        private static double GetMaxAffordableLoanAmountPurshase(double hoiYearly, double InterestRateYearly, double termInMonths, double downPayment, double maxMonthlyPayment, double propertyTaxesYearly, double MiRatio, double upfrontFundingFee)
        {
        ...
        internal static double GetMaxAffordableLoanAmountRefi(double propertyTaxesYearly, double homeOwnersInsutanceYearly, double affordableMonthlyPayment, double interestRateYearly, double LTV, double homeValue, double MiFactor, double term)
        {
            double num2 = propertyTaxesYearly;
            num2 /= 12.0;
            double num3 = homeOwnersInsutanceYearly;
            num3 /= 12.0;
            double num4 = affordableMonthlyPayment;
            num4 = (num4 - num3) - num2;
            double num5 = interestRateYearly;
            num5 /= 100.0;
            num5 /= 12.0;
            double num6 = num5;
            double y = term * 12.0;
            double num8 = num4;
            double num9 = MiFactor / 12.0;
            if (LTV < 80.0)
            {
                num9 = 0.0;
            }
            double x = num6 + 1.0;
            double num11 = Math.Pow(x, y);
            double num12 = num6 * num11;
            double num13 = num11 - 1.0;
            return ((num8 * num13) / (num12 - (num9 * num13)));
        }
        ...
        {
            List<OptionSelected> list = new List<OptionSelected>();
            List<RefiOption> refiOptions = GetRefiOptions(db, app);
            int num = 1;
            foreach (RefiOption option in refiOptions)
            {
                OptionSelected item = new OptionSelected {
                    OptIntex = num
                };
        ...
        internal static List<PurchaseOption> GetPurchaseOptions(Mortgage MtgApp)
        {
            ...
            for (int i = 0; i < purshaseVariables.Count; i++)
            {
                if (((!MtgApp.MortgagedProperty.OwnerShipType.HasValue || ((((((OwnershipTypeEnum) MtgApp.MortgagedProperty.OwnerShipType) != OwnershipTypeEnum.Investment) || purshaseVariables[i].Investment) && ((((OwnershipTypeEnum) MtgApp.MortgagedProperty.OwnerShipType) != OwnershipTypeEnum.Primary_Residence) || purshaseVariables[i].PrimaryResidence)) && ((((OwnershipTypeEnum) MtgApp.MortgagedProperty.OwnerShipType) != OwnershipTypeEnum.Second_Home) || purshaseVariables[i].SecondHome))) && (!MtgApp.MortgagedProperty.PropertyTypeApp.HasValue || ((((((((PropertyTypeEnum) MtgApp.MortgagedProperty.PropertyTypeApp) != PropertyTypeEnum.Modular_Manufactured) || purshaseVariables[i].Manufactured) && ((((PropertyTypeEnum) MtgApp.MortgagedProperty.PropertyTypeApp) != PropertyTypeEnum.Single_Familly_Residence_1_unit) || purshaseVariables[i].SFR)) && ((((PropertyTypeEnum) MtgApp.MortgagedProperty.PropertyTypeApp) != PropertyTypeEnum.Multi_Familly_Residence_5_Plus) || purshaseVariables[i].MultiUnits)) && (((((PropertyTypeEnum) MtgApp.MortgagedProperty.PropertyTypeApp) != PropertyTypeEnum.Multi_Familly_Residence_2_units) || (purshaseVariables[i].MaxNumberOfUnits >= 2)) && ((((PropertyTypeEnum) MtgApp.MortgagedProperty.PropertyTypeApp) != PropertyTypeEnum.Multi_Familly_Residence_3_units) || (purshaseVariables[i].MaxNumberOfUnits >= 3)))) && ((((((PropertyTypeEnum) MtgApp.MortgagedProperty.PropertyTypeApp) != PropertyTypeEnum.Multi_Familly_Residence_4_units) || (purshaseVariables[i].MaxNumberOfUnits >= 4)) && (((((PropertyTypeEnum) MtgApp.MortgagedProperty.PropertyTypeApp) != PropertyTypeEnum.Condo) || purshaseVariables[i].Condo) && ((((PropertyTypeEnum) MtgApp.MortgagedProperty.PropertyTypeApp) != PropertyTypeEnum.MobileHomeWithLand) || purshaseVariables[i].MobileHome))) && ((((PropertyTypeEnum) MtgApp.MortgagedProperty.PropertyTypeApp) != PropertyTypeEnum.Townhome) || purshaseVariables[i].TownHome))))) && (((MtgApp.MortgageApplicant.Veteran != YesNoAns.No) || (purshaseVariables[i].LoanType != LoanTypeEnum.VA)) && ((MtgApp.MortgagedProperty.Rural != YesNoAns.No) || (purshaseVariables[i].LoanType != LoanTypeEnum.USRDA))))
                {
                    ...
                    item.MI_Upfront_Fee = new decimal?(GetUpFrontFee(purshaseVariables[i].UpfrontMI, item.LoanAmount.Value));
        ...
        internal static List<RefiOption> GetRefiOptions(CcsLocalDbContext db, Application app)
        {
            ...
            Label_03AE:
                ...
                decimal? firstMortgagePayment = app.FirstMortgagePayment;
                decimal? monthlyMortgageInsur = app.MonthlyMortgageInsur;
                item.OldMonthlyPaymentPrincipalInterest = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() - monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                if (app.PymtIncludesPropTaxes)
                {
                    firstMortgagePayment = item.OldMonthlyPaymentPrincipalInterest;
                    monthlyMortgageInsur = app.AnnualPropertyTaxes;
                    monthlyMortgageInsur = monthlyMortgageInsur.HasValue ? new decimal?(monthlyMortgageInsur.GetValueOrDefault() / 12.00M) : ((decimal?) (nullable9 = null));
                    item.OldMonthlyPaymentPrincipalInterest = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() - monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                }
                if (app.PymtIncludesHomeownersInsurance)
                {
                ...
                firstMortgagePayment = item.HI_MonthlyAmount;
                monthlyMortgageInsur = item.HI_MonthsReserves;
                item.HazardInsuranceReserves = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() * monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                item.TermInYears = new int?(refiVariables[i].newTermInYears);
                item.OriginationChargesPercent = new double?(refiVariables[i].OriginationPercent);
                double num13 = 0.0;
                item.TotalFixedFees = new decimal?((((((((refiVariables[i].ProcessingFee + refiVariables[i].UnderwritingFee) + refiVariables[i].AppraisalFee) + refiVariables[i].CreditReportFee) + refiVariables[i].ClosingEscrowFee) + refiVariables[i].EndorsementsReconveyanceFee) + refiVariables[i].MortgageRecordingfee) + refiVariables[i].TaxServiceFee) + refiVariables[i].FloodCertificationFee);
                ...
                if (num18 <= maxLoanLimit)
                {
                    ...
                    if (v.FinalAdjustment >= -1000f)
                    {
                        ...
                        double affordableMonthlyPayment = GetMaxPayment(refiVariables[i].MaxfrontDTI, refiVariables[i].MaxBacktDTI, monthlyIncome, monthlyDebt);
                        double num28 = GetMaxAffordableLoanAmountRefi((double) app.AnnualPropertyTaxes.Value, (double) app.AnnualHomeownersInsur.Value, affordableMonthlyPayment, pricing.interest, refiVariables[i].MaxLTV, (double) app.EstimatedHomeValue.Value, refiVariables[i].MiFactor, (double) refiVariables[i].newTermInYears);
                        maxLoanLimit = Math.Min(maxLoanLimit, num28);
                        num4 = (double) app.FirstMortgageBalance.Value;
                        item.InterestRate = new double?(pricing.interest);
                        ...
                        if (flag && flag2)
                        {
                            num43 = ((((double) item.LoanBalance.Value) * item.CurrentInterestRate.Value) + (((double) item.SecondMtgBalance.Value) * app.SecondMortgageInterestRate.Value)) / (((double) item.LoanBalance.Value) + ((double) item.SecondMtgBalance.Value));
                            interestRateSavings = item.InterestRate;
                            item.BlendedRateSavings = interestRateSavings.HasValue ? new double?(num43 - interestRateSavings.GetValueOrDefault()) : ((double?) (nullable12 = null));
                            interestRateSavings = item.BlendedRateSavings;
                            if ((interestRateSavings.GetValueOrDefault() > 0.0) && interestRateSavings.HasValue)
                            {
                                item.ShowBlendedRate = true;
                        ...
                        num17 = num4 + num15;
                        num18 = num17 / (1.0 - num16);
                        item.LoanAmount = new decimal?((decimal) num18);
                        interestRateSavings = item.CreditPoints;
                        num43 = (double) item.LoanAmount.Value;
                        nullable12 = interestRateSavings.HasValue ? new double?(interestRateSavings.GetValueOrDefault() * num43) : null;
                        item.Credit = new decimal?((decimal) nullable12.Value);
                        item.Discount = new decimal?((decimal) (pricing.Cost30Days * ((double) item.LoanAmount.Value)));
                        num19 = (num18 / homeValue) * 100.0;
                        if (num19 > 80.0)
                        {
                        ...
                        if (num18 <= maxLoanLimit)
                        {
                            num20 = (double) app.TotalOfMonthlyPaymentsOnDebtToPayOff.Value;
                            num21 = (maxLoanLimit * (1.0 - num16)) - num17;
                            if (app.LoanTypeRequested == LoanTypeRequestedEnum.DebtConsolidationPayOffCreditors)
                            {
                                item.TotalBalanceOfDebtToConsolidate = app.EstimateTotalDebtToPayOff;
                                item.ShowDebtConsal = true;
                            ...
                            num13 = 0.0;
                            num22 = (maxLoanLimit * (1.0 - num16)) - num17;
                            num23 = Math.Min(num22, refiVariables[i].MaxCashOut);
                            if (app.LoanTypeRequested == LoanTypeRequestedEnum.CashOutMortgage)
                            {
                                num13 = (double) app.CashOutRequested.Value;
                                item.Cashout = app.CashOutRequested.Value;
                            ...
                            if (maxLoanLimit >= ((double) item.LoanAmount.Value))
                            {
                                ...
                                item.PMI_MIP_VA_FFReserves = 0;
                                item.MI_Upfront_Fee = 0.00M;
                                double num30 = (num24 / homeValue) * 100.0;
                                item.LTV = num30;
                                item.CLTV = num30;
                                if ((app.Has2ndMortgage > HaveSecondMortgageEnum.No) && (((YesNoAns) app.PayOff2ndMortgage) == YesNoAns.No))
                                {
                                ...
                                int num31 = (int) (item.LTV * 1000000.0);
                                item.LTV = ((double) num31) / 1000000.0;
                                num31 = (int) (item.CLTV * 1000000.0);
                                item.CLTV = ((double) num31) / 1000000.0;
                                if (item.CLTV <= refiVariables[i].CLTV)
                                {
                                    if (refiVariables[i].LoanType == LoanTypeEnum.HARP)
                                    {
                                    ...
                                    firstMortgagePayment = item.OldMonthlyPaymentPrincipalInterest;
                                    monthlyMortgageInsur = item.MI_MonthlyAmount;
                                    item.TotalOldPrincipalInterestPaymentWithMI = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                    firstMortgagePayment = item.NewMonthlyPaymentPrincipalInterest;
                                    monthlyMortgageInsur = item.NewMI_MonthlyAmount;
                                    item.TotalNewPrincipalInterestPaymentWithMI = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                    double num33 = ((((double) item.TotalNewPrincipalInterestPaymentWithMI.Value) + (((double) app.AnnualPropertyTaxes.Value) / 12.0)) + (((double) app.AnnualHomeownersInsur.Value) / 12.0)) + (((double) app.AnnualHomeownersAssocDues.Value) / 12.0);
                                    if (affordableMonthlyPayment >= num33)
                                    {
                                        item.FrontDTI = (num33 / monthlyIncome) * 100.0;
                                        item.BackDTI = ((num33 + monthlyDebt) / monthlyIncome) * 100.0;
                                        decimal? nullable = 0M;
                                        if (app.EstimateTotalDebtToPayOff != 0M)
                                        {
                                            firstMortgagePayment = app.TotalOfMonthlyPaymentsOnDebtToPayOff;
                                            monthlyMortgageInsur = item.TotalBalanceOfDebtToConsolidate;
                                            firstMortgagePayment = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() * monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                            monthlyMortgageInsur = app.EstimateTotalDebtToPayOff;
                                            nullable = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() / monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        }
                                        firstMortgagePayment = item.TotalOldPrincipalInterestPaymentWithMI;
                                        monthlyMortgageInsur = item.TotalNewPrincipalInterestPaymentWithMI;
                                        firstMortgagePayment = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() - monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        monthlyMortgageInsur = nullable;
                                        item.MonthlySavings = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        firstMortgagePayment = item.TotalNewPrincipalInterestPaymentWithMI;
                                        monthlyMortgageInsur = item.MonthlySavings;
                                        item.NewPaymentPlusMonthlySavings = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        if (((YesNoAns) app.PayOff2ndMortgage) == YesNoAns.Yes)
                                        {
                                            firstMortgagePayment = item.TotalOldPrincipalInterestPaymentWithMI;
                                            monthlyMortgageInsur = app.SecondMortgagePayment;
                                        ...
                                        if (flag && flag2)
                                        {
                                            ...
                                            monthlyMortgageInsur = app.SecondMortgagePayment;
                                            numofMonthstoEscrowTaxes = item.MonthsPaidRemaining2nd;
                                            monthlyMortgageInsur = monthlyMortgageInsur.HasValue ? new decimal?(monthlyMortgageInsur.GetValueOrDefault() * numofMonthstoEscrowTaxes) : ((decimal?) (nullable9 = null));
                                            item.OldTotalAmountOfAllPaymentsToBeMade = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        }
                                        item.NewTotalAmountOfAllPaymentsToBeMade = item.NewMonthlyPaymentPrincipalInterest * item.TermInMonths;
                                        firstMortgagePayment = item.OldTotalAmountOfAllPaymentsToBeMade;
                                        monthlyMortgageInsur = item.NewTotalAmountOfAllPaymentsToBeMade;
                                        item.NetSavingsFromOldLoanToNewLoan = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() - monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        firstMortgagePayment = item.NetSavingsFromOldLoanToNewLoan;
                                        if ((firstMortgagePayment.GetValueOrDefault() > 0M) && firstMortgagePayment.HasValue)
                                        {
                                            item.ShowNetSaving = true;
                                        ...
                                        firstMortgagePayment = firstMortgagePayment.HasValue ? new decimal?(firstMortgagePayment.GetValueOrDefault() * numofMonthstoEscrowTaxes) : null;
                                        monthlyMortgageInsur = item.Credit;
                                        item.AdjustedLoanOriginationFee = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() - monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        firstMortgagePayment = item.LoanAmount;
                                        numofMonthstoEscrowTaxes = ((decimal) refiVariables[i].OriginationPercent) - ((decimal) item.CreditPoints.Value);
                                        if (firstMortgagePayment.HasValue)
                                        {
                                        ...
                                        firstMortgagePayment = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        monthlyMortgageInsur = item.DailyInterestCharges;
                                        item.EstimatedPrepaidsReserves = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        firstMortgagePayment = item.ProcessingFee;
                                        monthlyMortgageInsur = item.UnderwritingFee;
                                        firstMortgagePayment = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        monthlyMortgageInsur = item.AppraisalFee;
                                        ...
                                        firstMortgagePayment = numofMonthstoEscrowTaxes + item.EstClosingCosts;
                                        monthlyMortgageInsur = item.EstimatedPrepaidsReserves;
                                        item.TotalAmountNeeded = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() + monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        firstMortgagePayment = item.LoanAmount;
                                        monthlyMortgageInsur = item.TotalAmountNeeded;
                                        item.EstimatedFundsNeeded = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() - monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        firstMortgagePayment = item.EstClosingCosts;
                                        monthlyMortgageInsur = item.MonthlySavings;
                                        item.CostSavingsBreakEvenAnalysis = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() / monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        double num35 = (double) item.LoanAmount.Value;
                                        interestRateSavings = item.InterestRate;
                                        double num36 = (interestRateSavings.Value / 100.0) / 12.0;
                                        double num37 = (double) app.FirstMortgagePayment.Value;
                                        if (((YesNoAns) app.PayOff2ndMortgage) == YesNoAns.Yes)
                                        {
                                            num37 += (double) app.SecondMortgagePayment.Value;
                                        }
                                        double a = -Math.Log(1.0 - ((num35 / num37) * num36)) / Math.Log(1.0 + num36);
                                        item.NumberofPaymentstoMaturity = new int?((int) Math.Ceiling(a));
                                        maxNumberOfUnits = item.NumberofPaymentstoMaturity;
                                        float? nullable14 = maxNumberOfUnits.HasValue ? new float?(((float) maxNumberOfUnits.GetValueOrDefault()) / 12f) : null;
                                        item.YearsRequiredToMaturity = nullable14.HasValue ? new double?((double) nullable14.GetValueOrDefault()) : null;
                                        item.AcceleratedPayoffTotalAmountOfAllPayments = item.NewPaymentPlusMonthlySavings * item.NumberofPaymentstoMaturity;
                                        firstMortgagePayment = item.OldTotalAmountOfAllPaymentsToBeMade;
                                        monthlyMortgageInsur = item.AcceleratedPayoffTotalAmountOfAllPayments;
                                        item.TotalSavingsFromOldMortgageToNewMortgage = (firstMortgagePayment.HasValue & monthlyMortgageInsur.HasValue) ? new decimal?(firstMortgagePayment.GetValueOrDefault() - monthlyMortgageInsur.GetValueOrDefault()) : ((decimal?) (nullable9 = null));
                                        maxNumberOfUnits = item.MonthsPaidRemaining;
                                        numberofPaymentstoMaturity = item.NumberofPaymentstoMaturity;
                                        ...
                                        {
                                            int num41 = getMILTV(loanAmount, (double) item.TermInMonths.Value, item.InterestRate.Value, (double) item.NewMonthlyPaymentPrincipalInterest.Value, 78.0, homeValue);
                                            payment = (((((double) item.NewMonthlyPaymentPrincipalInterest.Value) + ((double) item.NewMI_MonthlyAmount.Value)) * num41) + (((double) item.NewMonthlyPaymentPrincipalInterest.Value) * (((double) item.TermInMonths.Value) - num41))) / 360.0;
                                        }
                                        item.APR = new double?(GetAPR(loanAmount, item.InterestRate.Value / 1200.0, (double) item.TermInMonths.Value, payment));
                                        item.PercentCharge /= 100.0;
                                        item.ShowBlendedRate = true;
        ...
        internal static bool PullCredit(CcsLocalDbContext db, int applicantId, RequestCred requestcred)
        {
            ...
            {
                cred.Liabilities = MarkDublicateLiabilities(liabilities);
            }
            if (mortgages.Count > 0)
            {
                cred.CreditMortgages = MarkDublicateMortgages(mortgages);
            }
            else
            {
            ...
            applicant.lates12Credit = getLates30_12mo(applicant.Lates);
            applicant.lates24Credit = getLates30_24mo(applicant.Lates);
            PaymentAndBalance totalPaymentAndBalance = GetTotalPaymentAndBalance(applicant.CreditMortgages, applicant.CreditLiabilities);
